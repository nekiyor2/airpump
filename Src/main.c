...
#include "flash_config.h" // запис налаштувань у память
#include "battery.h"
#include "xgzp6847d.h" // датчик тиску i2c
#include "compressor.h" // bldc motor
#include "valve.h" // клапан для спуску тиску
#include "control.h" // логіка та контроль
#include "nrf24l01.h" // радіомодуль
...
int main(void)
{
  ...
  printf("System start\r\n");
   ...
   ControlConfig_t saved_cfg;
   if (FlashConfig_Load(&saved_cfg) == HAL_OK)
   {
       Control_SetConfig(&saved_cfg);
  ...
  while (1)
  {
	  // 1. Читаємо датчики
	        ...
	        if (NRF24_DataReady())
	        {
	            ...
	            Control_UpdateRxTick(); // оновлюємо час останнього зв'язку
	        ...
	        HAL_IWDG_Refresh(&hiwdg); // годуємо watchdog
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA, valve_Pin|GPIO_Output_CE_Pin|GPIO_Output_CSN_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : GPIO_Output_CE_Pin GPIO_Output_CSN_Pin */
  GPIO_InitStruct.Pin = GPIO_Output_CE_Pin|GPIO_Output_CSN_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
